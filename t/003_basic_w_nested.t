...

{
    ...
    is(${$b->current_buffer->string_ref}, 'KTHNXBYE', '... what we expected in the buffer');

    {
        ...
        $b->print('OH NOES');
        is($data, '', '... no more data is sent to the handle yet');
        is(${$b->current_buffer->string_ref}, 'OH NOES', '... what we expected in the buffer');

        $b->commit;
    }
    is($data, '', '... no more data is sent to the handle yet');
    is(${$b->current_buffer->string_ref}, 'KTHNXBYEOH NOES', '... what we expected in the buffer');
